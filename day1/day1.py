...
def loopThroughInput(input, idx):
    for i in input:
        variableName.append(input[idx]-i)
    idx += 1
    if idx >= len(input):
        pass
    else:
        loopThroughInput(input, idx)
    

loopThroughInput(input, idx)

def sumThree(input, loopThroughInput):
    loopThroughInput(input, idx)
    for k in input:
        if (versionTwo(input) - k) in variableName:
            print('success')
        else:
            pass

sumThree(input, loopThroughInput)"""